...
ws = (components nid)#0
elapsedTime numericalNoetherianOperators(I', Sampler => sampler, DependentSet => {1, 2, 4, 5, 6} / (i -> S_i), InterpolationTolerance => 1e-12, NoetherianDegreeLimit => 2)

ws = (components nid)#1
elapsedTime numericalNoetherianOperators(I', Sampler => sampler, DependentSet => {0, 1, 2, 3, 4} / (i -> S_i), InterpolationTolerance => 1e-6, NoetherianDegreeLimit => 3)
